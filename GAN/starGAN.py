...
class StarGAN():
    def __init__(self, image_size=(128, 128, 3), n_class=5, repeat_num=6, diters=5, D_lr=0.0002, G_lr=0.0002, lamda_gp=10, lamda_cls=1, lamda_rec=10):
        ...
        # train step
        self.D_train, self.G_train = self.starGAN_train(D_lr, G_lr, lamda_gp, lamda_cls, lamda_rec)
    ...
    def fit(self, X_path, y, batch_size=64, epochs=10000, draw=False, shuffle=True):
        ...
        for i in range(epochs):
            ...
            while j < batches:
                ...
                # train generator
                gen_iterations += 1
                [g_loss] = self.G_train([x_real, label_real, label_real_matrix, label_fake, label_fake_matrix])
    ...
    def Discriminator(self):
        ...
        x = Conv2D(64, (4, 4), strides=2, padding='same', kernel_initializer=conv_init)(input)
        x = LeakyReLU(alpha=0.01)(x)

        k_size=self.image_size[0]//2
        filters=128
        for i in range(self.repeat_num-3):
            x = Conv2D(filters, (3, 3), strides=2, padding='same', kernel_initializer=conv_init)(x)
            filters*=2
            k_size//=2
            x = LeakyReLU(alpha=0.2)(x)

        # k_size*k_size*1 conv filters with valid padding to get 1-d output, which is used to instead of the sigmoid
        output_x = Conv2D(1, (k_size, k_size), strides=1, padding='valid', kernel_initializer=conv_init, use_bias=False)(x)
        output_y = Conv2D(self.n_class, (k_size, k_size), strides=1, padding='valid', kernel_initializer=conv_init, use_bias=False)(x)
