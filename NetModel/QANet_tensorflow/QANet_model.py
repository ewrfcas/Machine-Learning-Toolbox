...
class Model(object):
    ...
    def build_model(self):
        ...
        with tf.variable_scope("Output_Layer"):
            start_logits = tf.squeeze(conv(tf.concat([enc[1], enc[2]], axis=-1), 1, bias=False, name="start_pointer"), -1)
            end_logits = tf.squeeze(conv(tf.concat([enc[1], enc[3]], axis=-1), 1, bias=False, name="end_pointer"), -1)
            ...
            # output
            outer = tf.matmul(tf.expand_dims(tf.nn.softmax(self.logits1), axis=2), tf.expand_dims(tf.nn.softmax(self.logits2), axis=1))
