...
def dot_attention(inputs, scope='dot_attention'):
    inputs_shape = inputs.get_shape().as_list()
    with tf.variable_scope(scope):
        W_s = tf.get_variable(name='W_sigmoid',
                              dtype=tf.float32,
                              shape=(inputs_shape[-1], inputs_shape[-1]),
                              regularizer=regularizer,
                              initializer=initializer())
        gate = tf.nn.sigmoid(dot(inputs, W_s))
        return inputs * gate

def gate_attention(inputs, memory, mask, hidden, scope="gate_attention"):
    ...
    with tf.variable_scope(scope):
        WC = tf.get_variable(name='WC',
                             dtype=tf.float32,
                             shape=(int(inputs_shape[-1]), hidden),
                             regularizer=regularizer,
                             initializer=initializer())
        WQ = tf.get_variable(name='WQ',
                             dtype=tf.float32,
                             shape=(int(memory_shape[-1]), hidden),
                             regularizer=regularizer,
                             initializer=initializer())
        V = tf.get_variable(name='V',
                            dtype=tf.float32,
                            shape=(int(2 * memory_shape[-1]), 2 * hidden),
                            regularizer=regularizer,
                            initializer=initializer())
        inputs_ = tf.nn.relu(dot(inputs, WC))
        memory_ = tf.nn.relu(dot(memory, WQ))
        outputs = tf.matmul(inputs_, tf.transpose(memory_, [0, 2, 1])) / (hidden ** 0.5)
        mask = tf.tile(tf.expand_dims(mask, axis=1), [1, inputs_shape[1], 1])
        logits = tf.nn.softmax(softmax_mask(outputs, mask))
        outputs = tf.matmul(logits, memory)
        res = tf.concat([inputs, outputs], axis=2)
        gate = tf.nn.sigmoid(dot(res, V))
        return res * gate
